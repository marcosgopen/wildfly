...
/**
 * Setup task that enables the LRA coordinator and participant subsystems
 * with a custom context-path ({@code "/"}) for the coordinator.
 */
public class EnableLRASetupTaskWithCustomContextPath extends SnapshotServerSetupTask {
    static final String CUSTOM_CONTEXT_PATH = "/";
    ...
    protected void doSetup(final ManagementClient managementClient, final String containerId) throws Exception {
        ...
        if (!subsystems.contains(SUBSYSTEM_LRA_COORDINATOR)) {
            ModelNode addOp = Operations.createAddOperation(Operations.createAddress("subsystem", SUBSYSTEM_LRA_COORDINATOR));
            addOp.get("context-path").set(CUSTOM_CONTEXT_PATH);
        ...
        if (!subsystems.contains(SUBSYSTEM_LRA_PARTICIPANT)) {
            // Derive the coordinator URL from the server's web URI so it works
            // regardless of port offsets or bind address changes
            String coordinatorUrl = managementClient.getWebUri() + "/lra-coordinator";
            ModelNode addOp = Operations.createAddOperation(Operations.createAddress("subsystem", SUBSYSTEM_LRA_PARTICIPANT));
            addOp.get("lra-coordinator-url").set(coordinatorUrl);
            builder.addStep(addOp);
        }
